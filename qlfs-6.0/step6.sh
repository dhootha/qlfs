...
then
	...
	echo "# QLFS 6.0" >> /boot/grub/menu.lst && \
	echo "title   QLFS 6.0, kernel 2.6.8.1" >> /boot/grub/menu.lst && \
	echo "root    (hd$dev,$par)" >> /boot/grub/menu.lst && \
	echo "kernel  /boot/lfskernel-2.6.8.1 root=$path ro" >> /boot/grub/menu.lst && \
	echo "" && \
...
then
	...
	umount /mnt/lfs &> /dev/null && \
	echo "" >> /etc/lilo.conf && \
	echo "image=/boot/lfskernel-2.6.8.1 " >> /etc/lilo.conf && \
	echo "label=QLFS 6.0" >> /etc/lilo.conf && \
